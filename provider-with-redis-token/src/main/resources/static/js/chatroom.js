...
function refreshUserList(){
    ...
    getData(host + "/user/online").then(online=>{
        for(var i=0;i<online.data.length;i++){
            var li = document.createElement("li");
            setUserData(online.data[i], li, true);
            user_box.appendChild(li);
        }
        return online;
    }).then(online=>{
        if(online.errorMsg==="OK"){
            getData(host + "/user/offline").then(offline=>{
                    for(var i=0;i<offline.data.length;i++){
                        var li = document.createElement("li");
                        setUserData(offline.data[i], li, false);
                        user_box.appendChild(li);
                    }
